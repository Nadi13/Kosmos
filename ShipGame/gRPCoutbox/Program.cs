...
var connString = "Host=localhost;Username=postgres;Password=0000;Database=outbox";
await using var conn = new NpgsqlConnection(connString);
await conn.OpenAsync();
var serverData = client.Order();
await using (var cmd = new NpgsqlCommand("SELECT serialized_message FROM transaction_messages WHERE is_sent=false", conn))
...
await foreach (OrderReply reply in replies)
{
    ...
    await using (var cmd = new NpgsqlCommand("UPDATE transaction_messages SET is_sent=true WHERE \"serialized_message\"=(@p)", conn))
    {
